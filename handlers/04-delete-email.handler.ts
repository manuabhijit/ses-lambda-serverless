...
import { DELETE_PASSWORD } from '../config/ses-password.config';

export const h04DeleteEmailHandler: Handler = (event: APIGatewayEvent, context: Context, cb: Callback) => {
  ...
  if(sesPassword !== DELETE_PASSWORD){
    let response = {
      statusCode: 401,
